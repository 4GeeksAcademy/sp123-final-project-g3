...
const TaskListItem = ({ task, onToggle, onDelete }) => (
    <li className="list-group-item d-flex justify-content-between align-items-center task-list-item" style={{backgroundColor: 'rgba(255, 255, 255, 0.2)'}}>
        <span 
...
export const Dashboard = () => {
    ...
    const [taskType, setTaskType] = useState("user"); // "user" o "clan"
    const [newTaskTitle, setNewTaskTitle] = useState("");

    
    const pendingUserTasks = store.userTasks.filter(t => !t.completed);
    const activeClan = store.clans.find(c => c.id === store.activeClanId);
    const activeClanTasks = store.clanTasks.filter(t => t.clanId === store.activeClanId && !t.completed);
    ...
    const handleAddTask = (e) => {
        ...
        if (taskType === 'user') {
            ...
            if (!store.activeClanId) {
                alert("Primero debes seleccionar un clan activo.");
                return;
            }
    ...
    const mockMembers = [
        ...
        { id: 3, name: store.profile.name, email: store.profile.email, role: "Owner", avatar: store.profile.avatar },
    ];
    const handleSendInvite = (e) => { e.preventDefault(); console.log("Invitado:", inviteEmail); };
    const handleCopyProjectLink = () => { navigator.clipboard.writeText(projectLink); alert("Enlace copiado"); };
